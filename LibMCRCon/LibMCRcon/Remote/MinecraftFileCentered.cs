...
namespace LibMCRcon.Remote
{
    public class MinecraftFileCentered : MinecraftFile
    {
        ...
        public void Recenter(int NewXo, int NewZo, int RegionsPerWorld )
        {
            ...
            X = (this.X + ((NewXo - 256) * this.RegionsPerWorld));
            Z = (this.Z + ((NewZo - 256) * this.RegionsPerWorld));
        ...
        public void CreateContributionList()
        {
            ...
            R.X -= (256 * RegionsPerWorld);
            R.Z -= (256 * RegionsPerWorld);
        ...
        public Bitmap RetrieveBitmap(MinecraftTransfer DL, string FileName)
        {
            ...
            {
                if (DL.TransferNext(FileName, ms, TransferDirection.RECEIVE))
        ...
        public void SaveBitmap(MinecraftTransfer DL, string FileName, Bitmap Bitmap)
        {
            using(MemoryStream ms = new MemoryStream())
            {
                ...
                ms.Position = 0;
                DL.TransferNext(FileName, ms, TransferDirection.SEND);
                ms.Close();
            }
  ...
  //          b2.Save(Path.Combine(ContentPath, unqBitMap));
        ...
        public static void RegionList(List<MinecraftFile> ContributionList, List<MinecraftFile> MFList)
        {
            foreach (var x in ContributionList)
                RegionList(x.X, x.Z, x.RegionsPerWorld, MFList);
        }
