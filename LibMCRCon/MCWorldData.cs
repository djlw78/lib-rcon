...
namespace LibMCRcon.WorldData
{
    ...
    public class Voxel
    {
        ...
        public bool ParseSegment(string CsvYXZ)
        {
            float tf = 0;
        ...
        public bool ParseSegment(string CsvYXZ, params int[] Offset)
        {
    ...
    public static class MinecraftOrdinates
    {
        ...
        public static int[] WorldMapRenderBoundries(string WorldThumbPrefix, string ImgsPath)
        {
            ...
            int A = Math.Abs(ULV[0].X - LRV[1].X);
            int B = Math.Abs(ULV[0].Z - LRV[1].Z);

            return new int[] { maxX, maxZ, ULV[0].X, ULV[0].Z, LRV[1].X, LRV[1].Z, A, B };
        ...
        public static Voxel[] WorldMapBoundriesFromImg(string WorldMapImg)
        {
            ...
            string[] ParseData = Path.GetFileNameWithoutExtension(WorldMapImg).Split('.');
            //0 - Type of thumb (WorldTopo64, WorldTile64, WorldTopo32, etc)
            //1 - World Thumb coordinate - X 
            //2 - World Thumb coordinate - Y
            //3 - left most x
